...
import { learnMoreSectionDataJapan } from '@/constant/countryHomePageData/japan';
  ...
  return (
    <div>
      <Banner
        ...
        price="$126.67"
        link="/japan/step-one"
        pageTitle={learnMoreSectionDataJapan?.pageTitle}
        pageName={learnMoreSectionDataJapan?.pageName}
      />
      {/* <ProcessingStep
      ...
      <div className="flex flex-col md:flex-row">
        <main className="flex-1 py-2 [&_strong]:text-tertiary [&_p]:font-normal [&_p]:text-[#343a40] [&_li]:text-[#343a40] [&_.link]:text-primaryMain [&_.email-link]:text-primaryMain">
          <LearnMore learnMoreSectionData={learnMoreSectionDataJapan} />
        </main>
        <aside className="basis-[300px] py-5 md:py-12 space-y-4">
          <div className="container">
            ...
            <ul className="flex flex-col gap-3">
              {learnMoreSectionDataJapan?.otherLinks?.map((link, index) => (
                <li key={index}>
      ...
      <Faq faqData={learnMoreSectionDataJapan?.faqData} />
      {/* Remember: For the most accurate and up-to-date information, it's best to check the official website. */}
      <PageReview applyLink={learnMoreSectionDataJapan?.applyNowLink} />
      <RelatedArticlesSlider
        relatedArticles={learnMoreSectionDataJapan.relatedArticles}
      />
    </div>
  );
};
