...
export default function Page({ params }) {
  ...
  if (getQuery.isSuccess) {
    ...
    const currentPerson = thailandVisaApplicationData?.persons?.find(
      person => person?._id === params?.id
    );

    const { _id, updatedAt, createdAt, __v, formId, ...currentPersonData } =
      currentPerson;
    ...
    return (
      <div>
        <div className="container  md:py-8 py-20 md;px-0 px-3 ">
          ...
          <div>
            <Formik
              initialValues={currentPersonData}
                    ...
                    <table className="w-full text-left border table-auto min-w-max">
                      ...
                      <tbody>
                        {thailandVisaApplicationData?.persons?.length > 0 ? (
                          thailandVisaApplicationData?.persons
                            ?.filter(person => person?._id !== params?.id)
                            ?.map(person => (
                              <tr key={person._id}>
                                <td className="p-4">
                                  <div
